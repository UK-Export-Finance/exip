...
const {
  ROOT,
  EXPORTER_BUSINESS: { ALTERNATIVE_TRADING_ADDRESS_ROOT, COMPANY_DETAILS, NATURE_OF_BUSINESS_ROOT },
} = INSURANCE_ROUTES;
...
const { REGISTERED_OFFICE_ADDRESS_HINT, REGISTERED_OFFICE_ADDRESS_HEADING } = FIELD_STRINGS[FULL_ADDRESS];

const address = application.COMPANY[COMPANY_ADDRESS];
const { DIFFERENT_TRADING_ADDRESS } = application;

context(
  'Insurance - Your business - Alternative trading address page - I want to input information on an alternative business trading address So that I can provide necessary business information to support my application for Export Insurance',
  () => {
    let referenceNumber;
