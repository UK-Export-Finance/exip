...
const invalidPageNumber = 2;

context('Insurance - Dashboard - pagination - user navigates to the dashboard with a page number param that does not exist', () => {
  let applications;
  ...
  before(() => {
    cy.completeSignInAndGoToDashboard().then(({ accountId }) => {
      ...
      cy.navigateToUrl(`${dashboardPageUrl}/${invalidPageNumber}`);
    });
  });
