...
context(
  'Insurance - Eligibility - Companies house number page - I want to check if I can use online service to apply for UKEF Export Insurance Policy for my export transaction if I do not have UK Companies House Registration Number - submit `no companies house number`',
  () => {
    const url = `${baseUrl}${COMPANIES_HOUSE_NUMBER}`;

    before(() => {
      cy.completeAndSubmitEligibilityForms({ form: 'exporterLocation' });
    });
