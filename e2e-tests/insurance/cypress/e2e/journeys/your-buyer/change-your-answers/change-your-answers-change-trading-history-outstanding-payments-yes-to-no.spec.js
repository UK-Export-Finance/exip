...
const { OUTSTANDING_PAYMENTS, TOTAL_AMOUNT_OVERDUE, TOTAL_OUTSTANDING_PAYMENTS } = FIELD_IDS;

const {
  ROOT,
  YOUR_BUYER: { TRADING_HISTORY, TRADING_HISTORY_CHANGE, CHECK_YOUR_ANSWERS },
} = INSURANCE_ROUTES;
...
context(
  `Insurance - Your buyer - Change your answers - Trading history - ${OUTSTANDING_PAYMENTS} - Yes to no - As an exporter, I want to change my answers to the trading history section`,
  () => {
    let referenceNumber;
    let url;
    ...
    before(() => {
      cy.completeSignInAndGoToApplication({}).then(({ referenceNumber: refNumber }) => {
        ...
        cy.completeAndSubmitYourBuyerForms({ form: 'buyerFinancialInformation', exporterHasTradedWithBuyer: true });
