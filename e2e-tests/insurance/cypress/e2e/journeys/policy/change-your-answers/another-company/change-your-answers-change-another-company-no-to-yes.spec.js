...
const {
  REQUESTED_JOINTLY_INSURED_PARTY: { REQUESTED: FIELD_ID, COMPANY_NAME, COMPANY_NUMBER, COUNTRY_CODE },
} = POLICY_FIELD_IDS;

const {
  ROOT,
  POLICY: { ANOTHER_COMPANY_CHANGE, OTHER_COMPANY_DETAILS_CHANGE, CHECK_YOUR_ANSWERS },
} = INSURANCE_ROUTES;

const baseUrl = Cypress.config('baseUrl');

context(
  'Insurance - Policy - Change your answers - Another company - No to yes - As an exporter, I want to change my answers to the other company details section',
  () => {
    let referenceNumber;
    ...
    describe('after changing the answer from no to yes', () => {
      ...
      it(`should redirect to ${OTHER_COMPANY_DETAILS_CHANGE} and then ${CHECK_YOUR_ANSWERS} after completing (now required) other company details fields`, () => {
        ...
        cy.assertChangeAnswersPageUrl({ referenceNumber, route: OTHER_COMPANY_DETAILS_CHANGE, fieldId: FIELD_ID });
