...
const {
  YOUR_COMPANY: { HAS_DIFFERENT_TRADING_NAME, HAS_DIFFERENT_TRADING_ADDRESS, YOUR_BUSINESS },
...
const companyDetails = {
  ...
  [HAS_DIFFERENT_TRADING_ADDRESS]: {
    label: () => cy.get(`[data-cy="${HAS_DIFFERENT_TRADING_ADDRESS}-legend"]`),
    noRadioInput: () => noRadioInput().eq(1),
    yesRadioInput: () => yesRadioInput().eq(1),
    errorMessage: () => cy.get(`[data-cy="${HAS_DIFFERENT_TRADING_ADDRESS}-error-message"]`),
  },
};
