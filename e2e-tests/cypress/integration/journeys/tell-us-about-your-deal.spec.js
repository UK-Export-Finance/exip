  ...
  describe('with valid login', () => {
    describe('rendering', () => {
      ...
      it('renders `credit limit group` label and hint', () => {
        const fieldId = CONSTANTS.FIELDS.CREDIT_LIMIT_GROUP;
      ...
      it('renders `credit limit currency` hidden label and input', () => {
        const fieldId = CONSTANTS.FIELDS.CREDIT_LIMIT_CURRENCY;
      ...
      it('renders `credit limit` hidden label and input', () => {
        const fieldId = CONSTANTS.FIELDS.CREDIT_LIMIT;
      ...
      it('renders `pre credit period` label, hint and input', () => {
        const fieldId = CONSTANTS.FIELDS.PRE_CREDIT_PERIOD;
      ...
      it('renders `credit period` label, hint and input', () => {
        const fieldId = CONSTANTS.FIELDS.CREDIT_PERIOD;
      ...
      it('renders `policy length` label, hint and input', () => {
        const fieldId = CONSTANTS.FIELDS.POLICY_LENGTH;
      ...
      it('renders `policy type` label, hint and radio inputs', () => {
        const fieldId = CONSTANTS.FIELDS.POLICY_TYPE;
    ...
    describe('form submission', () => {
      ...
      describe('when submitting an empty form', () => {
        it('should render validation errors for all required fields', () => {
          ...
          partials.errorSummaryListItems().eq(0).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.CREDIT_LIMIT_GROUP].IS_EMPTY;

            expect(text.trim()).equal(expectedMessage);
          });

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_LIMIT_GROUP].errorMessage().invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.CREDIT_LIMIT_GROUP].IS_EMPTY;
          ...
          partials.errorSummaryListItems().eq(1).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.CREDIT_PERIOD].IS_EMPTY;

            expect(text.trim()).equal(expectedMessage);
          });

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_PERIOD].errorMessage().invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.CREDIT_PERIOD].IS_EMPTY;
          ...
          partials.errorSummaryListItems().eq(2).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.POLICY_LENGTH].IS_EMPTY;

            expect(text.trim()).equal(expectedMessage);
          });

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_LENGTH].errorMessage().invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.POLICY_LENGTH].IS_EMPTY;
          ...
          partials.errorSummaryListItems().eq(3).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.POLICY_TYPE];

            expect(text.trim()).equal(expectedMessage);
          });

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_TYPE].errorMessage().invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.POLICY_TYPE];
      ...
      describe('when no `credit limit currency` is provided, but credit limit is', () => {
        it('should render a validation error', () => {
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_LIMIT_CURRENCY].input().select('AED');
          tellUsAboutYourDealPage.submitButton().click();
      ...
      describe('when no `credit limit` is provided, but `credit limit currency` is', () => {
        it('should render a validation error', () => {
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_LIMIT].input().type('100');
          tellUsAboutYourDealPage.submitButton().click();
      ...
      describe('when `credit limit` has a non-numeric value', () => {
        it('should render a validation error', () => {
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_LIMIT_CURRENCY].input().select('AED');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_LIMIT].input().type('a');
          tellUsAboutYourDealPage.submitButton().click();

          partials.errorSummaryListItems().eq(0).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.CREDIT_LIMIT].NOT_A_NUMBER;

            expect(text.trim()).equal(expectedMessage);
          });

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_LIMIT].errorMessage().invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.CREDIT_LIMIT].NOT_A_NUMBER;
      ...
      describe('when (optional field) `pre-credit period` has a non-numeric value', () => {
        it('should render a validation error', () => {
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.PRE_CREDIT_PERIOD].input().type('a');
          tellUsAboutYourDealPage.submitButton().click();

          partials.errorSummaryListItems().eq(1).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.PRE_CREDIT_PERIOD].NOT_A_NUMBER;

            expect(text.trim()).equal(expectedMessage);
          });

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.PRE_CREDIT_PERIOD].errorMessage().invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.PRE_CREDIT_PERIOD].NOT_A_NUMBER;
      ...
      describe('when `credit period` and `policy length` have non-numeric values', () => {
        it('should render validation errors', () => {
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_PERIOD].input().type('a');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_LENGTH].input().type('a');
          tellUsAboutYourDealPage.submitButton().click();

          partials.errorSummaryListItems().eq(1).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.CREDIT_PERIOD].NOT_A_NUMBER;

            expect(text.trim()).equal(expectedMessage);
          });

          partials.errorSummaryListItems().eq(2).invoke('text').then((text) => {
            const expectedMessage = ERROR_MESSAGES[CONSTANTS.FIELDS.POLICY_LENGTH].NOT_A_NUMBER;
      ...
      describe('with any validation error', () => {
        it('should render submitted values', () => {
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_LIMIT_CURRENCY].input().select('AED');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.PRE_CREDIT_PERIOD].input().type('0');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_PERIOD].input().type('1');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_LENGTH].input().type('2');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_TYPE].multi.input().click();

          tellUsAboutYourDealPage.submitButton().click();

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.PRE_CREDIT_PERIOD].input()
            .should('have.attr', 'value', '0');

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_PERIOD].input()
            .should('have.attr', 'value', '1');

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_LENGTH].input()
            .should('have.attr', 'value', '2');

          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_TYPE].multi.input().should('be.checked');
        });
      });
      ...
      describe('when form is valid', () => {
        it(`should redirect to ${CONSTANTS.ROUTES.CHECK_YOUR_ANSWERS}`, () => {
          ...
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.CREDIT_PERIOD].input().type('1');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_LENGTH].input().type('2');
          tellUsAboutYourDealPage[CONSTANTS.FIELDS.POLICY_TYPE].single.input().click();
