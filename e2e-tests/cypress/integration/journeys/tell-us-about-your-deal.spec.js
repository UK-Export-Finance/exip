...
context('Tell us about your deal page', () => {
  describe('rendering', () => {
    ...
    it('renders `credit limit group` label and hint', () => {
      const fieldId = FIELD_IDS.CREDIT_LIMIT_GROUP;
    ...
    it('renders `credit limit currency` hidden label and input', () => {
      const fieldId = FIELD_IDS.CREDIT_LIMIT_CURRENCY;
    ...
    it('renders `credit limit` hidden label and input', () => {
      const fieldId = FIELD_IDS.CREDIT_LIMIT;
    ...
    it('renders `policy length` label, hint and input', () => {
      const fieldId = FIELD_IDS.POLICY_LENGTH;
  ...
  describe('form submission', () => {
    ...
    describe('when submitting an empty form', () => {
      it('should render validation errors for all required fields', () => {
        ...
        partials.errorSummaryListItems().eq(0).invoke('text').then((text) => {
          const expectedMessage = ERROR_MESSAGES[FIELD_IDS.CREDIT_LIMIT_GROUP].IS_EMPTY;

          expect(text.trim()).equal(expectedMessage);
        });

        tellUsAboutYourDealPage[FIELD_IDS.CREDIT_LIMIT_GROUP].errorMessage().invoke('text').then((text) => {
          const expectedMessage = ERROR_MESSAGES[FIELD_IDS.CREDIT_LIMIT_GROUP].IS_EMPTY;
        ...
        partials.errorSummaryListItems().eq(2).invoke('text').then((text) => {
          const expectedMessage = ERROR_MESSAGES[FIELD_IDS.POLICY_LENGTH].IS_EMPTY;

          expect(text.trim()).equal(expectedMessage);
        });

        tellUsAboutYourDealPage[FIELD_IDS.POLICY_LENGTH].errorMessage().invoke('text').then((text) => {
          const expectedMessage = ERROR_MESSAGES[FIELD_IDS.POLICY_LENGTH].IS_EMPTY;
    ...
    describe('when no `credit limit currency` is provided, but credit limit is', () => {
      it('should render a validation error', () => {
        tellUsAboutYourDealPage[FIELD_IDS.CREDIT_LIMIT_CURRENCY].input().select('AED');
        tellUsAboutYourDealPage.submitButton().click();
    ...
    describe('when no `credit limit` is provided, but `credit limit currency` is', () => {
      it('should render a validation error', () => {
        tellUsAboutYourDealPage[FIELD_IDS.CREDIT_LIMIT].input().type('100');
        tellUsAboutYourDealPage.submitButton().click();
    ...
    describe('when `credit limit` has a non-numeric value', () => {
      it('should render a validation error', () => {
        tellUsAboutYourDealPage[FIELD_IDS.CREDIT_LIMIT_CURRENCY].input().select('AED');
        tellUsAboutYourDealPage[FIELD_IDS.CREDIT_LIMIT].input().type('a');
        tellUsAboutYourDealPage.submitButton().click();

        partials.errorSummaryListItems().eq(0).invoke('text').then((text) => {
          const expectedMessage = ERROR_MESSAGES[FIELD_IDS.CREDIT_LIMIT].NOT_A_NUMBER;

          expect(text.trim()).equal(expectedMessage);
        });

        tellUsAboutYourDealPage[FIELD_IDS.CREDIT_LIMIT].errorMessage().invoke('text').then((text) => {
          const expectedMessage = ERROR_MESSAGES[FIELD_IDS.CREDIT_LIMIT].NOT_A_NUMBER;
    ...
    describe('when `credit period` and `policy length` have non-numeric values', () => {
      it('should render validation errors', () => {
        ...
        tellUsAboutYourDealPage[FIELD_IDS.POLICY_LENGTH].input().type('a');
        tellUsAboutYourDealPage.submitButton().click();
        ...
        partials.errorSummaryListItems().eq(2).invoke('text').then((text) => {
          const expectedMessage = ERROR_MESSAGES[FIELD_IDS.POLICY_LENGTH].NOT_A_NUMBER;
    ...
    describe('with any validation error', () => {
      it('should render submitted values', () => {
        ...
        tellUsAboutYourDealPage[FIELD_IDS.POLICY_LENGTH].input().type('2');
        ...
        tellUsAboutYourDealPage[FIELD_IDS.POLICY_LENGTH].input()
          .should('have.attr', 'value', '2');

        tellUsAboutYourDealPage[FIELD_IDS.POLICY_TYPE].multi.input().should('be.checked');
      });
    });
