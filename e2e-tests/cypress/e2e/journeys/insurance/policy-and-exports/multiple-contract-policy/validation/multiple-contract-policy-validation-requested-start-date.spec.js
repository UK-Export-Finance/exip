...
context('Insurance - Policy and exports - Multiple contract policy page - form validation - requested start date', () => {
  ...
  describe('when day is not provided', () => {
    it('should render a validation error', () => {
      ...
      checkText(
        ...
        CONTRACT_ERROR_MESSAGES.MULTIPLE[REQUESTED_START_DATE].IS_EMPTY,
      );

      checkText(
        field.errorMessage(),
        `Error: ${CONTRACT_ERROR_MESSAGES.MULTIPLE[REQUESTED_START_DATE].IS_EMPTY}`,
      );
    });
  });
  ...
  describe('when day is not a number', () => {
    it('should render a validation error', () => {
      ...
      checkText(
        partials.errorSummaryListItems().eq(0),
        CONTRACT_ERROR_MESSAGES.MULTIPLE[REQUESTED_START_DATE].NOT_A_NUMBER,
      );

      checkText(
        field.errorMessage(),
        `Error: ${CONTRACT_ERROR_MESSAGES.MULTIPLE[REQUESTED_START_DATE].NOT_A_NUMBER}`,
      );
    });
  });
  ...
  describe('when the date is not in the future', () => {
    it('should render a validation error', () => {
      ...
      checkText(
        partials.errorSummaryListItems().eq(0),
        CONTRACT_ERROR_MESSAGES.MULTIPLE[REQUESTED_START_DATE].BEFORE_EARLIEST,
      );

      checkText(
        field.errorMessage(),
        `Error: ${CONTRACT_ERROR_MESSAGES.MULTIPLE[REQUESTED_START_DATE].BEFORE_EARLIEST}`,
      );
    });
  });
});
