...
const checkValidation = {
  day: {
    notProvided: () => {
      ...
      cy.checkText(field.errorMessage(), `Error: ${CONTRACT_ERROR_MESSAGES[REQUESTED_START_DATE].INCORRECT_FORMAT}`);
    },
    notANumber: () => {
      cy.keyboardInput(field.dayInput().clear(), 'Test');
      submitButton().click();

      cy.checkText(partials.errorSummaryListItems().eq(0), CONTRACT_ERROR_MESSAGES[REQUESTED_START_DATE].NOT_A_NUMBER);

      cy.checkText(field.errorMessage(), `Error: ${CONTRACT_ERROR_MESSAGES[REQUESTED_START_DATE].NOT_A_NUMBER}`);
    },
  },
  month: {
    notProvided: () => {
  ...
  notInTheFuture: () => {
    ...
    cy.checkText(partials.errorSummaryListItems().eq(0), CONTRACT_ERROR_MESSAGES[REQUESTED_START_DATE].BEFORE_EARLIEST);

    cy.checkText(field.errorMessage(), `Error: ${CONTRACT_ERROR_MESSAGES[REQUESTED_START_DATE].BEFORE_EARLIEST}`);
  },
  invalidFormat: () => {
    const now = new Date();
    const day = now.getDate();
