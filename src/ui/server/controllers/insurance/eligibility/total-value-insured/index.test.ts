...
const {
  ELIGIBILITY: { TOTAL_VALUE_INSURED_CHANGE, COVER_PERIOD, CHECK_YOUR_ANSWERS },
} = INSURANCE_ROUTES;
  ...
  describe('post', () => {
    ...
    describe('when there are no validation errors', () => {
      ...
      it(`should redirect to ${COVER_PERIOD}`, () => {
        ...
        expect(res.redirect).toHaveBeenCalledWith(COVER_PERIOD);
      });

      describe("when the url's last substring is `change`", () => {
        it(`should redirect to ${CHECK_YOUR_ANSWERS}`, async () => {
          req.originalUrl = TOTAL_VALUE_INSURED_CHANGE;
