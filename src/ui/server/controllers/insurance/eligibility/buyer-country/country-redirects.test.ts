...
const {
  ELIGIBILITY: { CANNOT_APPLY_EXIT: CANNOT_APPLY_ROUTE, TOTAL_VALUE_INSURED, BUYER_COUNTRY_CHANGE, CHECK_YOUR_ANSWERS, TALK_TO_AN_EXPORT_FINANCE_MANAGER },
} = INSURANCE_ROUTES;
...
describe('controllers/insurance/eligibility/buyer-country - redirects', () => {
  ...
  describe('post', () => {
    ...
    describe('when the submitted country does not have short term cover', () => {
      ...
      it(`should redirect to ${TALK_TO_AN_EXPORT_FINANCE_MANAGER}`, async () => {
        await post(req, res);

        expect(res.redirect).toHaveBeenCalledWith(TALK_TO_AN_EXPORT_FINANCE_MANAGER);
      });
    });
