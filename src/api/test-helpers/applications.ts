...
const applicationQuery =
  'id createdAt updatedAt referenceNumber dealType submissionCount submissionDeadline submissionType status previousStatus version eligibility { id } policy { id } exportContract { id } owner { id } company { id } business { id businessContactDetail { id } } broker { id } buyer { id } sectionReview { id } declaration { id } policyContact { id }';
...
const get = async ({ context, applicationId }: TestHelperApplicationGet): Promise<Application> => {
  try {
    ...
    const application = (await context.query.Application.findOne({
      ...
      query: 'id eligibility { id } buyer { id } referenceNumber',
    })) as Application;
